...
def process_api(
    ...
    if is_gif(target_path) and is_gif(output_path):
        temp_target_path = target_path[:-4] + '.mp4'
        temp_output_path = output_path[:-4] + '.mp4'
        gif_to_video(target_path, temp_target_path)
        os.remove(target_path)
    parse_api_args(source_path, temp_target_path, temp_output_path, frame_processors, keep_fps, keep_audio, keep_frames, many_faces, mouth_mask, nsfw_filter, map_faces, video_encoder, video_quality, live_mirror, live_resizable, max_memory, execution_provider, execution_threads)
    if not pre_check():
        return False
    for frame_processor in get_frame_processors_modules(modules.globals.frame_processors):
        if not frame_processor.pre_check():
    ...
    start()
    if is_gif(target_path) and is_gif(output_path):
        video_to_gif(temp_output_path, output_path)
...
def parse_api_args(
    ...
    modules.globals.output_path = normalize_output_path(modules.globals.source_path, modules.globals.target_path, output_path)
    modules.globals.frame_processors = frame_processors
    modules.globals.headless = source_path or target_path or output_path
    ...
    modules.globals.live_mirror = live_mirror
    modules.globals.live_resizable = live_resizable
    modules.globals.max_memory = max_memory
    modules.globals.execution_providers = decode_execution_providers(execution_provider)
    modules.globals.execution_threads = execution_threads
